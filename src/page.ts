...
<html lang="en">
  ...
  <body>
    <header>
      <h1>tuya-matter</h1>
      <button id="refresh">Refresh from Tuya</button>
    </header>
    ...
    <footer>
      <a
        href="https://github.com/caarlos0/matter-tuya"
        target="_blank"
        rel="noreferrer"
      >
    ...
    <script type="module">
      ...
      function capabilities(device) {
        const parts = [...device.quantities];
        if (device.switchable) parts.unshift("on/off");
        return parts.length ? parts.join(", ") : "nothing to expose";
      }
